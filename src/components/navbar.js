...
export function renderNavbar() {
  ...
  const isPdf = route.startsWith('/tool/') && ['protect-pdf','unlock-pdf','pdf-metadata','jpg-to-pdf','merge-pdf','split-pdf','pdf-to-word','word-to-pdf','compress-pdf','acrobat-downloader','pdf-to-images'].includes(route.replace('/tool/',''));
  const isSocial = route.startsWith('/tool/') && ['thumbnail-grabber','tweet-generator','youtube-tags','instagram-post','x-image-slicer','instagram-downloader','youtube-downloader'].includes(route.replace('/tool/',''));
